...
/**
 * Interfaccia di facciata (Facade) per l'interazione tra livello UI e logica applicativa
 * del sistema FleetManager.
 * <p>
 * Questa interfaccia espone un insieme di operazioni di alto livello utilizzate dai controller
 * JavaFX per gestire veicoli, prenotazioni, manutenzioni, scadenze, notifiche e utenti.
 * Le implementazioni concrete orchestrano i vari gestori di servizio e delegano la persistenza
 * allo strato repository/DAO.
 * </p>
 */
public interface UiFacade {
     ...
     * @return lista delle prenotazioni del driver (eventualmente vuota, mai {@code null})
     */
     ...
     * @return lista delle prenotazioni con lo stato indicato (eventualmente vuota, mai {@code null})
     */
    List<Prenotazione> getPrenotazioniByStato(StatoPrenotazione stato);

    /**
     * Crea una nuova prenotazione per un driver e un veicolo in un determinato intervallo temporale.
     *
     ...
     * @param utente utente che richiede l'annullamento (driver o manager a seconda delle regole di business)
     */
    ...
    /**
     * Aggiorna gli stati delle prenotazioni in base alle regole temporali e di business.
     */
     ...
     *
     * @return lista completa delle manutenzioni (eventualmente vuota, mai {@code null})
     */
     ...
     * <p>
     * Metodo esposto per evitare che la UI contenga logica applicativa: in precedenza
     * questa operazione era gestita nel controller.
     * </p>
     */
     ...
     *
     * @return lista completa delle notifiche (eventualmente vuota, mai {@code null})
     */
     ...
     * @return lista delle notifiche dell'utente (eventualmente vuota, mai {@code null})
     */
     ...
     *
     * @param idUtente identificativo dell'utente
     * @return lista delle notifiche non lette (eventualmente vuota, mai {@code null})
     */
    ...
    /**
     * Restituisce le notifiche visibili a un utente:
     * tutte se l'utente è un manager, solo le proprie se l'utente è un driver.
     *
     * @param u utente di riferimento
     * @return lista delle notifiche visibili (eventualmente vuota, mai {@code null})
     */
    List<Notifica> getNotifichePerUtente(Utente u);

    /**
     * Invia una segnalazione straordinaria relativa a un veicolo, effettuata da un driver.
     *
    ...
    /**
     * Restituisce le prenotazioni visibili all'utente loggato (tutte se manager, solo proprie se driver),
     * già ordinate secondo le regole applicative.
     *
     * @param utenteLoggato utente per il quale calcolare la visibilità delle prenotazioni
     * @return lista delle prenotazioni visibili, già ordinata (eventualmente vuota, mai {@code null})
     */
    List<Prenotazione> getPrenotazioniVisibiliOrdinare(Utente utenteLoggato);

    /**
     * Restituisce una cache utile alla UI per risolvere rapidamente gli utenti a partire dal loro id.
     *
     * @return mappa {@code idUtente -> Utente} (eventualmente vuota, mai {@code null})
     */
